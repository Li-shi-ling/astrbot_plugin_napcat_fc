...
- 将 NapCat 入口权限从 AstrBot 管理员校验改为消息发送者 ID 过滤，不再调用 `event.is_admin()` 判断权限。
- 新增 `enable_sender_id_filter` 和 `allowed_sender_ids` 配置：开关关闭时不限制发送者；开关开启时只允许列表内 `event.get_sender_id()` 使用搜索和调用入口。
- 明确回复消息不会影响权限判断，发送者过滤只使用本条消息发送人的 ID，并补充对应回归测试。

## v1.16.4 - 2026-05-11

- 恢复 `admin_only` 配置项，默认开启管理员限制，避免插件配置页缺失管理员限制开关。
- `napcat_search_tools` 和 `napcat_call_tool` 改为读取 `admin_only`：开启时仍只允许 AstrBot 管理员使用，关闭后非管理员也可使用这两个入口。
- 更新 README、配置 schema、工具发现报告和回归测试，覆盖默认开启、关闭后允许非管理员搜索/调用的场景。

## v1.16.3 - 2026-04-29

- 删除旧版单会话无限工具注入、单次添加上限和持久化发现队列写入/读取逻辑，搜索工具继续只返回调用说明，不注入具体 API 工具。
- 新增 `search_result_suppress_turns` 配置：同一会话中已经搜索返回过的工具会在指定对话轮数内被排除，配置小于等于 `0` 时会一直排除到聊天历史清空。
- 新增基于 `req.contexts` 历史数量回退的清空检测，聊天记录被清理后自动重置该会话的搜索屏蔽队列。
- 新增管理员权限校验：`napcat_search_tools` 和 `napcat_call_tool` 仅允许 AstrBot 管理员使用，非管理员调用时返回权限不足提示。
- 新增回归测试覆盖搜索屏蔽轮数、永久屏蔽和历史清空重置，以及管理员权限通过/拒绝场景。

## v1.16.2 - 2026-04-28

- 将 `pipe` 和 `tsv` 搜索结果从轻量摘要改为完整文本调用说明，包含工具用途、接口名、风险信息、完整参数说明、必填参数、调用样例和 `napcat_call_tool` 使用说明。
- 更新 `search_result_format` 配置说明、README 和工具发现报告，明确轻量搜索暂不作为当前设计目标。
- 新增回归测试覆盖默认 `pipe` 与 `tsv` 的完整参数说明输出。
